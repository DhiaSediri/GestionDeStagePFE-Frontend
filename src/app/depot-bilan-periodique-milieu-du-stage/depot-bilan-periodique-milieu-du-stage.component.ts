...
export class DepotBilanPeriodiqueMilieuDuStageComponent implements OnInit {
  ...
  upload(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.depotService.uploadFileDepotBilan_périodique_milieu_du_stage(this.currentFile, this.currentUser.email).subscribe(
          (event: any) => {
    ...
    this.depotService.addDepotBilan_périodique_milieu_du_stageToRemote(this.currentUser.id).subscribe(
      (resultat) => {
  ...
  emailSenderUploadDepotToEncadrant(depotBilan_périodique_milieu_du_stage: Depot) {
    this.https.post<Depot>('http://localhost:8081/emailSender/getdetailsUploadDepotToEncadrant', depotBilan_périodique_milieu_du_stage).subscribe(
      res => {
  ...
  getFileList(): void {
    this.depotService.listFileDepotBilan_périodique_milieu_du_stage(this.currentUser.email).subscribe(result => {
  ...
  deleteFile(fileData: FileData): void {
    ...
    this.depotService.deleteFileDepotBilan_périodique_milieu_du_stage(fileData.filename, this.currentUser.email).subscribe(
      blob => saveAs(blob, fileData.filename));
      alert('Cette opération a été effectuée avec succès');
      this._router.navigate(['Bilan_periodique_milieu_du_stage']);

    this.depotService.deleteDepotBilan_périodique_milieu_du_stageByEtudiantIdFromRemote(this.currentUser.id).subscribe(
  ...
  getFile(fileData: FileData) {
    this.depotService.getPdfBilan_périodique_milieu_du_stage(fileData.filename, this.currentUser.email).subscribe((responseMessage) => {
  ...
  getCommentaireBilan_periodique_milieu_du_stageList(): void {
    ...
    this.commentaireService.fetchCommentaireBilan_periodique_milieu_du_stageListFromRemote(this.currentUser.id).subscribe(   
      (data: Commentaire[]) => {
  ...
  goToEditCommentaireBilan_periodique_milieu_du_stage(id : number){
    console.log("id "+id);
    this._router.navigate(['/editCommentaireBilan_periodique_milieu_du_stage', id]);
  }

  deleteCommentaireBilan_periodique_milieu_du_stage(id : number){
